...
export default function UpdateClubName({params}: {params: {clubId: string}}) {
    ...
    const [data, setData] = useState<dataForm>({ name: "", club: params.clubId})
    ...
    async function handleSubmit(e: FormEvent) {
        e.preventDefault()
        console.log(data)
        const res = await fetch("https://club.jactc.xyz/api/v1/club/club/update/name", {
            method: "PATCH",
            mode: "cors",
            headers: {
            ...
            <section>
                ...
                <article className="max-w-[250px] flex justify-center m-auto">
                    <form className="flex flex-col gap-3" onSubmit={handleSubmit}>
                        <input className="px-2 py-1 rounded-md text-black" required onChange={handleChange} name="name" placeholder="New Name" type="text" />
